...
GST_DEBUG_CATEGORY_STATIC (gst_aja_hevc_src_debug);
#define GST_CAT_DEFAULT gst_aja_hevc_src_debug

#define DEFAULT_MODE            (GST_AJA_MODE_HEVC_720_420_8_5994p)
#define DEFAULT_CONNECTION      (GST_AJA_VIDEO_CONNECTION_AUTO)
...
static GstStaticPadTemplate gst_aja_hevc_src_template = GST_STATIC_PAD_TEMPLATE ("src",
                                                                                 GST_PAD_SRC,
                                                                                 GST_PAD_ALWAYS,
...
typedef struct
{
    GstAjaHevcSrc               *hevc_src;
    AjaVideoBuff                *video_buff;
    GstClockTime                capture_time;
    GstClockTime                capture_duration;
    GstAjaModeHevcEnum          mode;
} AjaCaptureHevcFrame;

static void
aja_capture_hevc_frame_free (void *data)
{
    AjaCaptureHevcFrame *frame = (AjaCaptureHevcFrame *) data;

    //printf("aja_capture_hevc_frame_free\n");
    
    if ((frame->hevc_src->input) && (frame->hevc_src->input->ntv2AVHevc))
        frame->hevc_src->input->ntv2AVHevc->ReleaseVideoBuffer(frame->video_buff);
    g_free (frame);
}
...
static void
aja_hevc_frame_free (void *data)
{
    AjaHevcFrame *frame = (AjaHevcFrame *) data;

    printf("aja_hevc_frame_free\n");
...
static void gst_aja_hevc_src_set_property (GObject * object, guint property_id, const GValue * value, GParamSpec * pspec);
static void gst_aja_hevc_src_get_property (GObject * object, guint property_id, GValue * value, GParamSpec * pspec);

static void gst_aja_hevc_src_finalize (GObject * object);

static gboolean gst_aja_hevc_src_set_caps (GstBaseSrc * src, GstCaps * caps);
static GstCaps *gst_aja_hevc_src_get_caps (GstBaseSrc * src, GstCaps * filter);
static gboolean gst_aja_hevc_src_query (GstBaseSrc * src, GstQuery * query);
static gboolean gst_aja_hevc_src_unlock (GstBaseSrc * src);
static gboolean gst_aja_hevc_src_unlock_stop (GstBaseSrc * src);

static GstFlowReturn gst_aja_hevc_src_create (GstPushSrc * psrc, GstBuffer ** buffer);

static gboolean gst_aja_hevc_src_open (GstAjaHevcSrc * video_src);
static gboolean gst_aja_hevc_src_close (GstAjaHevcSrc * video_src);

static gboolean gst_aja_hevc_src_stop (GstAjaHevcSrc * src);

static void gst_aja_hevc_src_start_streams (GstElement * element);

static GstStateChangeReturn gst_aja_hevc_src_change_state (GstElement * element, GstStateChange transition);
static GstClock *gst_aja_hevc_src_provide_clock (GstElement * element);

static gboolean gst_aja_hevc_src_hevc_callback (int64_t refcon, int64_t msg);

#define parent_class gst_aja_hevc_src_parent_class
G_DEFINE_TYPE (GstAjaHevcSrc, gst_aja_hevc_src, GST_TYPE_PUSH_SRC);

static void
gst_aja_hevc_src_class_init (GstAjaHevcSrcClass * klass)
{
    ...
    gobject_class->set_property = gst_aja_hevc_src_set_property;
    gobject_class->get_property = gst_aja_hevc_src_get_property;
    gobject_class->finalize = gst_aja_hevc_src_finalize;

    element_class->change_state = GST_DEBUG_FUNCPTR (gst_aja_hevc_src_change_state);
    element_class->provide_clock = GST_DEBUG_FUNCPTR (gst_aja_hevc_src_provide_clock);

    basesrc_class->get_caps = GST_DEBUG_FUNCPTR (gst_aja_hevc_src_get_caps);
    basesrc_class->set_caps = GST_DEBUG_FUNCPTR (gst_aja_hevc_src_set_caps);
    basesrc_class->query = GST_DEBUG_FUNCPTR (gst_aja_hevc_src_query);
    basesrc_class->unlock = GST_DEBUG_FUNCPTR (gst_aja_hevc_src_unlock);
    basesrc_class->unlock_stop = GST_DEBUG_FUNCPTR (gst_aja_hevc_src_unlock_stop);
    
    pushsrc_class->create = GST_DEBUG_FUNCPTR (gst_aja_hevc_src_create);

    g_object_class_install_property (gobject_class, PROP_MODE,
                                     g_param_spec_enum ("mode", "Playback Mode",
                                                        "Video Mode to use for playback",
                                                        GST_TYPE_AJA_MODE_HEVC, DEFAULT_MODE,
    ...
    gst_element_class_add_pad_template (element_class, gst_static_pad_template_get (&gst_aja_hevc_src_template));

    gst_element_class_set_static_metadata (element_class, "Aja HEVC/H.265 Source", "Video/Src", "Aja HEVC/H.265 RT Video Encoder Source", "PSM <philm@aja.com>");
    
    GST_DEBUG_CATEGORY_INIT (gst_aja_hevc_src_debug, "ajavideosrc", 0, "debug category for ajavideosrc element");
}

static void
gst_aja_hevc_src_init (GstAjaHevcSrc *src)
{
...
void
gst_aja_hevc_src_set_property (GObject * object, guint property_id, const GValue * value, GParamSpec * pspec)
{
    GstAjaHevcSrc *src = GST_AJA_HEVC_SRC (object);
    GST_DEBUG_OBJECT (src, "set_property");

    switch (property_id)
    {
        case PROP_MODE:
            src->modeEnum = (GstAjaModeHevcEnum) g_value_get_enum (value);
            break;
...
void
gst_aja_hevc_src_get_property (GObject * object, guint property_id, GValue * value, GParamSpec * pspec)
{
    GstAjaHevcSrc *src = GST_AJA_HEVC_SRC (object);
...
void
gst_aja_hevc_src_finalize (GObject * object)
{
    GstAjaHevcSrc *src = GST_AJA_HEVC_SRC (object);
...
static gboolean
gst_aja_hevc_src_set_caps (GstBaseSrc * bsrc, GstCaps * caps)
{
    GstAjaHevcSrc *src = GST_AJA_HEVC_SRC (bsrc);
    ...
    mode = gst_aja_get_mode_hevc (src->modeEnum);
    g_assert (mode != NULL);
        
    g_mutex_lock (&src->input->lock);
    src->input->mode = mode;
    src->input->video_enabled = TRUE;
    gst_aja_hevc_src_start_streams(src->input->videosrc);
...
static GstCaps *
gst_aja_hevc_src_get_caps (GstBaseSrc * bsrc, GstCaps * filter)
{
    GstAjaHevcSrc *src = GST_AJA_HEVC_SRC (bsrc);
    ...
    mode_caps = gst_static_pad_template_get_caps (&gst_aja_hevc_src_template);
...
void
gst_aja_hevc_src_convert_to_external_clock (GstAjaHevcSrc * src, GstClockTime * timestamp, GstClockTime * duration)
{
    GstClock *clock;
...
static gboolean
gst_aja_hevc_src_query (GstBaseSrc * bsrc, GstQuery * query)
{
    GstAjaHevcSrc *src = GST_AJA_HEVC_SRC (bsrc);
...
static gboolean
gst_aja_hevc_src_unlock (GstBaseSrc * bsrc)
{
    GstAjaHevcSrc *src = GST_AJA_HEVC_SRC (bsrc);
...
static gboolean
gst_aja_hevc_src_unlock_stop (GstBaseSrc * bsrc)
{
    GstAjaHevcSrc *src = GST_AJA_HEVC_SRC (bsrc);
    ...
    g_mutex_lock (&src->lock);
    src->flushing = FALSE;
    g_queue_foreach (&src->current_frames, (GFunc) aja_capture_hevc_frame_free, NULL);
...
static gboolean
gst_aja_hevc_src_open (GstAjaHevcSrc * src)
{
    AJAStatus           status;
    const GstAjaMode *  mode;
    GST_DEBUG_OBJECT (src, "open");
    
    src->input = gst_aja_acquire_input (src->device_number, src->input_channel, GST_ELEMENT_CAST (src), FALSE, TRUE);
    if (!src->input)
    {
    ...
    src->input->ntv2AVHevc->SetCallback(VIDEO_CALLBACK, (int64_t)&gst_aja_hevc_src_hevc_callback, (int64_t)src);

    return TRUE;
}

static gboolean
gst_aja_hevc_src_close (GstAjaHevcSrc * src)
{
...
static gboolean
gst_aja_hevc_src_stop (GstAjaHevcSrc * src)
{
...
static void
gst_aja_hevc_src_start_streams (GstElement * element)
{
    GstAjaHevcSrc *src = GST_AJA_HEVC_SRC (element);
...
gst_aja_hevc_src_change_state (GstElement * element, GstStateChange transition)
{
    GstAjaHevcSrc *src = GST_AJA_HEVC_SRC (element);
...
static void
gst_aja_hevc_src_got_frame (GstAjaHevcSrc * src, AjaVideoBuff * videoBuff, GstAjaModeHevcEnum mode, GstClockTime capture_time, GstClockTime capture_duration)
{
    ...
    gst_aja_hevc_src_convert_to_external_clock (src, &capture_time, &capture_duration);
    //GST_ERROR_OBJECT (src, "Actual timestamp %" GST_TIME_FORMAT, GST_TIME_ARGS (capture_time));
    
    g_mutex_lock (&src->lock);
    if (!src->flushing)
    {
        AjaCaptureHevcFrame *f;
        
        while (g_queue_get_length (&src->current_frames) >= src->queue_size)
        {
            f = (AjaCaptureHevcFrame *) g_queue_pop_head (&src->current_frames);
            GST_WARNING_OBJECT (src, "Dropping old frame at %" GST_TIME_FORMAT,
                                GST_TIME_ARGS (f->capture_time));
            aja_capture_hevc_frame_free (f);
        }
        
        f = (AjaCaptureHevcFrame *) g_malloc0 (sizeof (AjaCaptureHevcFrame));
        f->hevc_src = src;
...
gst_aja_hevc_src_create (GstPushSrc * bsrc, GstBuffer ** buffer)
{
    GstAjaHevcSrc *src = GST_AJA_HEVC_SRC (bsrc);
    ...
    if (src->modeEnum != f->mode)
    {
        ...
        src->modeEnum = f->mode;
        g_mutex_unlock (&src->lock);
        caps = gst_static_pad_template_get_caps (&gst_aja_hevc_src_template);
    ...
    vf = (AjaHevcFrame *) g_malloc0 (sizeof (AjaHevcFrame));
    
    *buffer = gst_buffer_new_wrapped_full ((GstMemoryFlags) GST_MEMORY_FLAG_READONLY,
                                           (gpointer) data, data_size, 0, data_size, vf,
                                           (GDestroyNotify) aja_hevc_frame_free);
    
    vf->hevc_src = f->hevc_src;
    vf->video_buff = f->video_buff;
    if (vf->hevc_src->input)
        vf->hevc_src->input->ntv2AVHevc->AddRefVideoBuffer(vf->video_buff);
...
static GstClock *
gst_aja_hevc_src_provide_clock (GstElement * element)
{
    GstAjaHevcSrc *src = GST_AJA_HEVC_SRC (element);
...
static gboolean
gst_aja_hevc_src_hevc_callback(int64_t refcon, int64_t msg)
{
    GstAjaHevcSrc *src = (GstAjaHevcSrc *) refcon;
    AjaVideoBuff *videoBuffer = (AjaVideoBuff *) msg;
    
    if (src->input->video_enabled == FALSE)
        return FALSE;
    
    gst_aja_hevc_src_got_frame(src, videoBuffer, src->modeEnum, videoBuffer->timeStamp, videoBuffer->timeDuration);
    return TRUE;
}
