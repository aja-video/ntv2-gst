...
static GstStaticPadTemplate gst_aja_hevc_src_template =
...
static void gst_aja_hevc_src_set_property (GObject * object, guint property_id,
    const GValue * value, GParamSpec * pspec);
static void gst_aja_hevc_src_get_property (GObject * object, guint property_id,
...
static GstFlowReturn gst_aja_hevc_src_create (GstPushSrc * psrc,
    GstBuffer ** buffer);
...
static GstStateChangeReturn gst_aja_hevc_src_change_state (GstElement * element,
...
gst_aja_hevc_src_class_init (GstAjaHevcSrcClass * klass)
{
  ...
  element_class->change_state =
      GST_DEBUG_FUNCPTR (gst_aja_hevc_src_change_state);
  element_class->provide_clock =
      GST_DEBUG_FUNCPTR (gst_aja_hevc_src_provide_clock);
  ...
  gst_element_class_add_pad_template (element_class,
      gst_static_pad_template_get (&gst_aja_hevc_src_template));

  gst_element_class_set_static_metadata (element_class, "Aja HEVC/H.265 Source",
      "Video/Src", "Aja HEVC/H.265 RT Video Encoder Source",
      "PSM <philm@aja.com>");

  GST_DEBUG_CATEGORY_INIT (gst_aja_hevc_src_debug, "ajavideosrc", 0,
...
void
gst_aja_hevc_src_set_property (GObject * object, guint property_id,
    const GValue * value, GParamSpec * pspec)
{
...
void
gst_aja_hevc_src_get_property (GObject * object, guint property_id,
    GValue * value, GParamSpec * pspec)
{
  GstAjaHevcSrc *src = GST_AJA_HEVC_SRC (object);
...
void
gst_aja_hevc_src_convert_to_external_clock (GstAjaHevcSrc * src,
    GstClockTime * timestamp, GstClockTime * duration)
{
  ...
  if (clock && clock != src->input->clock) {
    ...
    gst_clock_get_calibration (src->input->clock, &internal, &external, &rate_n,
        &rate_d);
    ...
    external_start_time_diff =
        src->external_base_time - external_start_time_diff;
...
gst_aja_hevc_src_unlock_stop (GstBaseSrc * bsrc)
{
  ...
  g_mutex_lock (&src->lock);
  src->flushing = FALSE;
  g_queue_foreach (&src->current_frames, (GFunc) aja_capture_hevc_frame_free,
      NULL);
...
gst_aja_hevc_src_open (GstAjaHevcSrc * src)
{
  ...
  src->input =
      gst_aja_acquire_input (src->device_number, src->input_channel,
      GST_ELEMENT_CAST (src), FALSE, TRUE);
  if (!src->input) {
  ...
  status = src->input->ntv2AVHevc->Init (src->input->mode->videoPreset,
      ...
      false, true, src->input->mode->isQuad, false, false);
  if (!AJA_SUCCESS (status)) {
  ...
  src->input->ntv2AVHevc->SetCallback (VIDEO_CALLBACK,
      (int64_t) & gst_aja_hevc_src_hevc_callback, (int64_t) src);
...
gst_aja_hevc_src_start_streams (GstElement * element)
{
  ...
  if (src->input->video_enabled && (!src->input->audiosrc
    ...
    if (src->input->ntv2AVHevc) {
      ...
      src->external_base_time =
          gst_clock_get_internal_time (GST_ELEMENT_CLOCK (src));
...
static void
gst_aja_hevc_src_got_frame (GstAjaHevcSrc * src, AjaVideoBuff * videoBuff,
    GstAjaModeHevcEnum mode, GstClockTime capture_time,
    GstClockTime capture_duration)
{
  ...
  gst_aja_hevc_src_convert_to_external_clock (src, &capture_time,
      &capture_duration);
...
gst_aja_hevc_src_hevc_callback (int64_t refcon, int64_t msg)
{
  ...
  gst_aja_hevc_src_got_frame (src, videoBuffer, src->modeEnum,
      videoBuffer->timeStamp, videoBuffer->timeDuration);
  return TRUE;
}
